...
`vertex-move-endgame-mark` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies endgame behavior through round-trip fixtures, with lossless normalization checks and offline replay mode.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
